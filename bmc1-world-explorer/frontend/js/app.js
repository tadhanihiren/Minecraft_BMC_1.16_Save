...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const btnSetAsPlayer = document.getElementById("btnSetAsPlayer");
  ...
  // Right-click or double-click to set player location
  mapController.onSetPlayerLocation = (x, z, y) => {
    const elX = document.getElementById("nearX");
    const elZ = document.getElementById("nearZ");
    if (elX) elX.value = x;
    if (elZ) elZ.value = z;
    mapController.setPlayerMarker(x, z, y);
    showToast(`📍 Set player location: X ${x}, Z ${z}`);
  };
  ...
  if (btnSetAsPlayer) {
    btnSetAsPlayer.addEventListener("click", () => {
      if (!selectedLocation) return;
      const x = selectedLocation.x;
      const y = selectedLocation.y !== undefined ? selectedLocation.y : 64;
      const z = selectedLocation.z;
      const elX = document.getElementById("nearX");
      const elZ = document.getElementById("nearZ");
      if (elX) elX.value = x;
      if (elZ) elZ.value = z;
  ...
  async function loadCurrentWorld() {
    try {
      ...
      if (data && data.world_path) {
        ...
        try {
          const pRes = await fetch("/api/worlds/player");
          if (pRes.ok) {
            const pData = await pRes.json();
            if (pData && pData.status === "success") {
              const elX = document.getElementById("nearX");
              const elY = document.getElementById("nearY");
              const elZ = document.getElementById("nearZ");
              if (elX) elX.value = pData.x;
              if (elY) elY.value = pData.y;
              if (elZ) elZ.value = pData.z;
              if (pData.dimension === currentDimension) {
                mapController.setPlayerMarker(pData.x, pData.z, pData.y);
              }
            }
          }
        } catch (_) {}
