...
class MinecraftMap {
  ...
  _bindEvents() {
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      const x = Math.floor(mc.x);
      const z = Math.floor(mc.z);
      if (this.onMouseMove) {
    ...
    this.canvas.addEventListener("contextmenu", (e) => {
      ...
      const x = hit ? hit.x : Math.floor(mc.x);
      const z = hit ? hit.z : Math.floor(mc.z);
      const y = hit && hit.y !== undefined ? hit.y : 64;
      if (this.onSetPlayerLocation) {
        this.onSetPlayerLocation(x, z, y);
      } else {
        this.setPlayerMarker(x, z, y);
      }
    });
